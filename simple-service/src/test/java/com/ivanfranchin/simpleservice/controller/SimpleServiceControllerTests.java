...
class SimpleServiceControllerTests {
  ...
  @MockitoBean private JwtAuthenticationTokenConverter jwtAuthenticationTokenConverter;
  ...
  void shouldReturnPrivateStringWithJwt() throws Exception {
    mockMvc.perform(get("/api/private").with(jwt())).andExpect(status().isForbidden());
  }
