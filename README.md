...
The goal of this project is to create a simple REST API (`simple-service`) and securing it with `Keycloak`.
...
4. Export to a variable the client secret generated by Keycloak for `simple-service` (Configuring Keycloak, step 4).
```
export KEYCLOAK_CLIENT_SECRET=<credential generated by Keycloak for simple-service">
```
...
```
curl -i http://localhost:8080/api/private -H "Authorization: Bearer $MYCOMPANY_BGATES_ACCESS_TOKEN"
```
...
```
curl -i http://localhost:8080/api/private -H "Authorization: Bearer $MYCOMPANY_MCUBAN_ACCESS_TOKEN"
```
