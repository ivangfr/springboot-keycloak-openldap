...
```
curl -H "authorization: $MYCOMPANY_BGATES_ACCESS_TOKEN" "http://localhost:8080/api/private"
```
It will return:
```
...
4. Now click on `GET /api/private`, it is a secured endpoint. Let's try it without authentication.

5. Click on `Try it out` button and then on `Execute` button
It will return:
```
TypeError: Failed to fetch
```

6. In order to access the private endpoint, you need an access token. To get it, run the following commands in a terminal.
...
7. Copy (`Ctr-C`) the token generated (something like that starts with `Bearer ...`) and go back to `Swagger`

8. Click on the `Authorize` button, paste (`Ctr-V`) the copied token in the value field and. Then, click on `Authorize` and, to finalize, click on `Done`.

9. Go to `GET /api/private`, click on `Try it out` and then on `Execute` button
It will return:
```
Code: 200
...
10. The token default expiration period is `5 minutes`. So, wait this time and, using the same token, try to call the private endpoint.
It will return:
```
Code: 401
Response Body:
